...
var paramInput = flag.String("input", "", "input file dir")

func main() {

	flag.Parse()

	inputGoFileDir := *paramInput
	files, err := ioutil.ReadDir(inputGoFileDir)
	if nil != err {
	...
	for _, filename := range files {
		...
		//filename = *paramInput + "/" + filename
		file, err := parser.ParseFile(fs, *paramInput+"/"+filename.Name(), nil, parser.ParseComments)
...
// DO NOT EDIT!
import {MessageRegister} from "../../ccLib/codeC/messageRegister";
export class MessageHelper {
	static Init() {
		{{range .Structs}}
		MessageRegister.RegisterPbMessage("{{.Name}}",{{.MsgID}},message.{{.Name}}.encode,message.{{.Name}}.decode);
		{{end}}
	}
}
`
...
func genCode(output string, f *Package) {
	...
	generateCode("internal-gen", codeTemplate, output, f, &generateOption{formatGoCode: false})
}
